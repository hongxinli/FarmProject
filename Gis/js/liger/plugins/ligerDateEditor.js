...
(function ($) {
    $.fn.ligerDateEditor = function () {
        return $.liger.run.call(this, "ligerDateEditor", arguments);
    };

    $.fn.ligerGetDateEditorManager = function () {
        return $.liger.run.call(this, "ligerGetDateEditorManager", arguments);
    };
    ...
    $.liger.controls.DateEditor = function (element, options) {
        $.liger.controls.DateEditor.base.constructor.call(this, element, options);
    };
    $.liger.controls.DateEditor.ligerExtend($.liger.controls.Input, {
        __getType: function () {
            return 'DateEditor';
        },
        __idPrev: function () {
        ...
        showDate: function () {
           ...
           /* var dateStr = this.currentDate.year + '/' + this.currentDate.month + '/' + this.currentDate.date + ' ' + this.currentDate.hour + ':' + this.currentDate.minute;
            ...
            this.onTextChange();*/
            var mth = this.currentDate.month;//周峰修改，原来的处理有些浏览器不认识格式，按照统一规范处理
            if (mth >= 1)
                mth -= 1;
            var myDate = new Date(this.currentDate.year, mth, this.currentDate.date, this.currentDate.hour, this.currentDate.minute);
            dateStr = g.getFormatDate(myDate);
            this.inputText.val(dateStr);
            this.onTextChange();
        },
