...
(function ($) {
    ...
    //dialog 图片文件夹的路径 预加载
    $.liger.DialogImagePath = "../../lib/liger/skins/Aqua/images/win/";
    ...
    $.ligerDefaults.Dialog = {
        ...
        isMaskOcx: false,             //是否调用报表ocx遮挡
        isCompatibilityMode: false,     //兼容模式
        onLoaded: null,
        onExtend: null,
        onExtended: null,
        onCollapse: null,
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        _render: function () {
            ...
            if (p.target) {
               ...
               if (p.timeParmName) {
                ...
                if (p.load) {
                    ...
                    g.jiframeloading = $("<div class='l-dialog-loading' style='display:block;'></div>");
                    ...
                    if (p.isCompatibilityMode) {
                        g.dialog.body.removeAttr("style");
                        g.dialog.content.removeAttr("style");
                        g.jiframe.css("height", p.height);
                        g.jiframe.css("width", p.width);
                    }
                    g.dialog.content.prepend(g.jiframe);
                    g.dialog.content.prepend(g.jiframeloading);
                    g.dialog.content.addClass("l-dialog-content-nopadding");
                    setTimeout(function () {
                        g.jiframe.attr("src", p.url).bind("load.iframe", function () {
                            g.jiframeloading.hide();
                        });
            ...
            $(".l-dialog-tc .l-dialog-close", g.dialog).click(function () {
                if (p.isHidden) {
                    g.hide();
                    if (p.isMaskOcx && g.dialog.iframe)
                        g.dialog.iframe.hide();
                    //增加对关闭的处理
        ...
        maskOcx: function () {
            var g = this, p = this.options;
            g.dialog.iframe = $("<iframe frameborder='0' class='l-dialog-shadow' style='z-index:8900;position:absolute;'></iframe>").insertAfter(g.dialog);
            g.updateShadow(g.dialog)
        ...
        show: function () {
            ...
            //前端显示 
            $.liger.win.setFront.ligerDefer($.liger.win, 100, [g]);
            ...
            if ($.fn.ligerDrag)
            {
                g.draggable = g.dialog.ligerDrag({
                    ...
                    onStopDrag: function ()
                    {
                        ...
                        if (p.target)
                        {
                            var triggers1 = l.find($.liger.controls.DateEditor);
                            var triggers2 = l.find($.liger.controls.ComboBox);
                            //更新所有下拉选择框的位置
        ...
        updateShadow: function (dialog) {
            var g = this, p = this.options;
            if (!dialog.iframe) return false;
            dialog.iframe.css({
            ...
            if ($.fn.ligerResizable) {
                g.resizable = g.dialog.ligerResizable({
                    onStopResize: function (current, e) {
                        ...
                        if (p.isMaskOcx && g.dialog.iframe) {
                            g.dialog.iframe.css({
                                left: g.dialog.css('left'),
                                top: g.dialog.css('top'),
                                width: current.newWidth,
                                height: g.dialog.outerHeight()
                            });
                        }
                        return false;
                    }, animate: false
    ...
    $.ligerDialog.alert = function (content, title, type, op, callback) {
        ...
        var btnclick = function (item, Dialog, index) {
            Dialog.close();
            if (typeof Dialog.dialog.iframe != "undefined")
                Dialog.dialog.iframe.hide();
            if (callback)
    ...
    $.ligerDialog.confirm = function (content, title, op, callback) {
        if (typeof (title) == "function") {
            callback = title;
            type = null;
        }
    ...
    $.ligerDialog.warning = function (content, title, op, callback) {
    ...
    $.ligerDialog.closeWaitting = function () {
        ...
        for (var i in dialogs) {
            ...
            if (d.dialog.hasClass("l-dialog-waittingdialog")) {
                if (typeof d.dialog.iframe != "undefined")
                    d.dialog.iframe.hide();
                d.close();
            }
        }
    };
    ...
    $.ligerDialog.prompt = function (title, value, multi,op,callback) {
        ...
        if (typeof (op) == "object" || typeof (value) == "object" || typeof (multi) == "object")
           $.extend(p, op);
        return $.ligerDialog(p);
    };
