...
(function ($) {
    ...
    window.liger = $.liger = {
        ...
        run: function (plugin, args, ext) {
            ...
            if (this == null || this == window || ext.isStatic) {
                ...
                return new $.liger[ext.controlNamespace][plugin]($.extend({}, $[ext.defaultsNamespace][plugin] || {}, $[ext.defaultsNamespace][plugin + 'String'] || {}, args.length > 0 ? args[0] : {}));
            }
            if (!liger.plugins[plugin]) {
                liger.plugins[plugin] = {
            ...
            this.each(function () {
                ...
                if (ext.hasElement) {
                    new $.liger[ext.controlNamespace][plugin](this, p);
                }
                else {
                    new $.liger[ext.controlNamespace][plugin](p);
                }
            });
    ...
    $.extend(liger.core.Component.prototype, {
        ...
        trigger: function (arg, data) {
            ...
            for (var i = 0; i < event.length; i++) {
                var ev = event[i];
                return ev.handler.apply(ev.context, data);
    ...
    liger.core.UIComponent.ligerExtend(liger.core.Component, {
        ...
        _init: function () {
            ...
            //读取attr方法,并加载到参数,比如['url'],并修复插件本身属性为空的问题，如nullText
            var attributes = this.attr();
            if (attributes && attributes instanceof Array) {
                for (var i = 0; i < attributes.length; i++) {
                    var name = attributes[i];
                    this.options[name] = $(this.element).attr(name) || this.options[name];
                }
            }
            //读取liger这个属性，并加载到参数，比如 liger = "width:120,heigth:100"
            var p = this.options;
            if ($(this.element).attr("liger")) {
                try {
                    var attroptions = $(this.element).attr("liger");
        ...
        _rendered: function () {
            if (this.element) {
                $(this.element).attr("ligerid", this.id);
            }
        },
        //返回要转换成liger参数的属性,比如['url']
        attr: function () {
            return [];
        },
        destroy: function () {
    ...
    liger.win = {
        ...
        mask: function (win) {
            ...
            if (!this.windowMask) {
                ...
                $(window).bind('resize.ligerwin', setHeight);
        ...
        unmask: function (win) {
            ...
            for (var i = 0, l = jwins.length; i < l; i++) {
                var winid = jwins.eq(i).attr("ligerid");
        ...
        setFront: function (win) {
            ...
            for (var i in wins) {
                var w = wins[i];
                if (w == win) {
                    $(w.element).css("z-index", "92000");
                    this.activeTask(w);
                }
                else {
                    $(w.element).css("z-index", "91000");
                }
            }
        }
    };
        ...
        return $.extend({
            create: function (container, editParm, controlOptions) {
                ...
                if (field.editor) {
                    ...
                    $.extend(p, ext);
                    //扩展url和data，防止gird点击每列时候多次向服务请求数据
                    var url = field.editor.url;
                    url = typeof (url) == 'function' ? url() : url;
                    $.extend(p, { url: url });
                    var data = field.editor.bindData;
                    data = typeof (data) == 'function' ? data() : data;
                    $.extend(p, { data: data });
                    var griddata = field.editor.grid && field.editor.grid.bindData;
                    griddata = typeof (griddata) == 'function' ? griddata() : griddata;
                    $.extend(p.grid, { data: griddata });
                    var treedata = field.editor.tree && field.editor.tree.bindData;
                    treedata = typeof (treedata) == 'function' ? treedata() : treedata;
                    treedata&&$.extend(p.tree, { data: JSON2.parse(treedata) });
