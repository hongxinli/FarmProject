...
(function ($)
{
    ...
    {
        return $.liger.run.call(this, "ligerComboBox", arguments);
    };

    $.fn.ligerGetComboBoxManager = function ()
    {
        return $.liger.run.call(this, "ligerGetComboBoxManager", arguments);
    };
    ...
    $.ligerDefaults.ComboBox = {
        ...
        isParentDialog: false,     //是否父页面形式打开
        isImage: false,             //加载是否是图片   
        xScroll: true,
        absolute: true,         //选择框是否在附加到body,并绝对定位
        ...
        isCascadeTrigger: false,  //是否级联触发,默认不触发
        isMaskOcx: false,  //报表控件遮罩
        inputOnly: true //扩展，默认不能输入
    ...
    $.liger.controls.ComboBox = function (element, options)
    {
        $.liger.controls.ComboBox.base.constructor.call(this, element, options);
    };
    $.liger.controls.ComboBox.ligerExtend($.liger.controls.Input, {
        __getType: function ()
        {
            return 'ComboBox';
        },
        ...
        _init: function ()
        {
            $.liger.controls.ComboBox.base._init.call(this);
            var p = this.options;
            if (p.columns)
            {
        ...
        _render: function ()
        {
            ...
            if (this.element.tagName.toLowerCase() == "input")
            {
                ...
                {
                    $(this.element).hide();
                    g.inputText = $('<div  style="cursor:pointer;vertical-align:middle;"></div>');
            ...
            //下拉框
           // debugger;
            g.selectBox = $('<div class="l-box-select" style="overflow:auto" ><div class="l-box-select-inner" ><table id="' + p.selDivId + '" cellpadding="0" cellspacing="0" border="0" class="l-box-select-table"></table></div></div>');

          
            g.selectBox.table = $("table:first", g.selectBox);
            g.selectBox.css("z-index",90002);
            //外层
            ...
            //添加独立父页面弹出变量
            g.parentComboId = "ligercomo" + new Date().getTime();
            if (p.absolute)
            g.selectBox.appendTo('body').addClass("l-box-select-absolute").css("overflow","auto");
            else
                g.textwrapper.append(g.selectBox);
            //去掉下拉框横向滚动
            if (!p.isMultiSelect&&!p.xScroll)
               $(">div", g.selectBox).css({"overflow":"auto","overflow-x":"hidden"});
            ...
            $(document).bind("click.combobox", function (e) {
               var tag = (e.target || e.srcElement).tagName.toLowerCase();
                //var className = $.trim((e.target || e.srcElement).className.toLowerCase());
               var className = $.trim((e.target || e.srcElement).className);
               if (tag == "html" || tag == "body") {
                    g._toggleSelectBox(true);
               }
               if (className == "main_container" || className == "l-form-container l-form-containerlt" || className == "bq")
               {
                   g._toggleSelectBox(true);
               }
            });
            //增加阴影解决报表遮挡
            if(p.isMaskOcx) g.selectBox.shadow=$('<iframe class="l-selectbox-shadow" scroll=no style="position:absolute;z-index:90001"  frameborder=0></iframe>').insertAfter(g.selectBox).hide();
        },
        selectByParent:function()
        {
            var g = this, p = this.options;
            var cleft = $(".l-dialog-win:visible", parent.document).offset().left + g.wrapper.offset().left + 6;
            var ctop = $(".l-dialog-win:visible", parent.document).offset().top + g.wrapper.offset().top + 24 + 24;
            var id = "pbox" + new Date().getTime();
            var pcombox = $(parent.document).find("#" + g.parentComboId);
            

            if (pcombox.length == 0) {
                var pbox = g.selectBox.clone(true);
                pbox.hover(function () { }, function () {
                    $(this).hide();
                });
                $("td", pbox).bind("click", function () {
                    if (p.isImage)
                        g.inputText.html($(this).html());
                    pbox.hide();
                });
                
                pbox.attr("id", g.parentComboId).attr(g.parentComboId,0).css({ "position": "absolute", "z-index": "9300", left: cleft, top: ctop, "display": "block" });
                //修复ie6下父页面append不支持对象的问题
                if ($.browser.msie && ($.browser.version == "6.0")) {
                   $("body", parent.document).append(pbox[0].outerHTML);
                   pcombox = $(parent.document).find("#" + g.parentComboId);
                   $("td", pcombox).unbind("click");
                   $("td", pcombox).bind("click", function () {
                        if (p.isImage) {
                            g.inputText.html($(this).html());
                        } else {
                            g.inputText.val($(this).text());
                        }
                        g.valueField.val($(this).val());
                        pcombox.hide();
                    });
                }
                else {
                    $("body", parent.document).append(pbox);
                }
            }
            else {
                ...
                if (!p.isMultiSelect) {
                    ...
                    $("tr[value=" + g.valueField.val() + "] td", pcombox).addClass("l-selected");
                    $("tr[value!=" + g.valueField.val() + "] td", pcombox).removeClass("l-selected");
                    pcombox.css({ "position": "absolute", "z-index": 9300, left: cleft, top: ctop}).show();
        ...
        updateShadow: function () {
            var g = this, p = this.options;
            if (!g.selectBox.shadow) return false;
            g.selectBox.shadow.css({
                left: g.selectBox.css('left'),
                top: g.selectBox.css('top'),
                width: g.selectBox.outerWidth(),
                height: g.selectBox.outerHeight()
            });
        ...
        _setCancelable: function (value) {
            ...
            g.unselect.hover(function () {
                ...
                //修复下拉框树形点击右侧小差号后树形还选中的问题
                $(".l-checkbox-checked", g.selectBox).removeClass("l-checkbox-checked").addClass("l-checkbox-unchecked");
                g.clear();
            });
        },
        ...
        setData: function (data)
        {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if (!p.columns) {
                    ...
                    if (!p.isImage)
                        ...
                        out.push("<td index='" + i + "' value='" + val + "' align='left' height='15' ><img border=0 style='padding-top:10px' src='" + txt + "'/></td></tr>");
                } else {
            ...
            if (!p.columns) {
                if (p.isShowCheckBox) {
                    ...
                    if (out.length > 1000) {
                        var newOut = out.slice(0);
                        $("table.l-table-nocheckbox", g.selectBox).append(newOut.slice(0, 20).join(''));
                        $(">.l-box-select-inner", g.selectBox).scroll(function () {
                           if (newOut.length > 0) {
                                    newOut.splice(0, 20);
                                    $("table.l-table-nocheckbox", g.selectBox).append(newOut.slice(0, 20).join(''));
                        ...
                        $(">.l-box-select-inner", g.selectBox).unbind("scroll");
                    }
            ...
            $(".l-table-checkbox input:checkbox", g.selectBox).change(function ()
            {
                ...
                if (this.checked && g.hasBind('beforeSelect'))
                {
                    ...
                    if (parentTD != null && g.trigger('beforeSelect', [parentTD.attr("value"), parentTD.attr("text")]) == false)
                    {
                        ...
                        g.selectBox.shadow&&g.selectBox.shadow.hide();
                        return false;
                    }
                }
                if (!p.isMultiSelect)
            ...
            g._addClickEven();
            //选择项初始化
            if (!p.autocomplete && p.isCascadeTrigger) {
                g.updateStyle();
            }
        },
        //树
        setTree: function (tree)
        {
            ...
            g.inputText.val("");
            g.valueField.val("");
            $(".l-box-select-inner", g.selectBox).html("");
        ...
        {
            var g = this, p = this.options;
            $(g.grid.rows).each(function (i, row) {
                if (row[p.valueField] == value) g.grid.select(row,false);
            });
       ...
       //表格
            ...
            if (p.condition) {
                ...
                $("div", containerBtn1).ligerButton({
                    ...
                    click: function () {
                        ...
                        g.grid.setParm(grid.conditionParmName || 'condition', $.liger.toJSON(rules));
                        g.grid.reload();
                    }
                });
        ...
        _changeValue: function (newValue, newText)
        {
            ...
            {
                if (p.isImage)
                    g.inputText.append("<img src='" + newText + "'  style='padding-top:10px' />");
                else
                {
                    if(newText==""&&newValue!="")
                        g.inputText.val(newValue);
                    else
        ...
        updateSelectBoxPosition: function ()
        {
            ...
            if (p &&p.absolute) {
                ...
                if (Number(g.wrapper.offset().left + g.selectBox.width()) > contentWidth
            			&& contentWidth > Number(g.selectBox.width() + 1)) {
                    g.selectBox.css({ left: g.wrapper.offset().left -3- (Math.abs(g.selectBox.width() - g.wrapper.outerWidth())) });
                }
             }
            else {
        ...
        _toggleSelectBox: function (isHide)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    g.selectBox.shadow && g.selectBox.shadow.show();
                    g.selectBox.show();
                    g.boxToggling = false;
                    if (!g.tree && !g.grid && !p.isShowCheckBox && $('td.l-selected', g.selectBox).length > 0)
                    {
                        ...
                        if(p.silde)
                            ...
                            $(".l-box-select-inner", g.selectBox).animate({ scrollTop: offSet }, 0);
                    }
                }
            }
                    ...
                    if (p.url) {
                        ...
                        if (p.data != null) {
                            ...
                            for (var i = 0; i < p.data.length; i++) {
                                if (p.data[i][p.textField].indexOf(g.inputText.val()) != -1) {
                                    node.push(p.data[i]);
                                    if (p.data[i][p.textField] == g.inputText.val())
                                        g._changeValue(p.data[i][p.valueField], p.data[i][p.textField])
                                }
                            }
                            g.setData(node);
                        } else {
    ...
    $.liger.controls.ComboBox.prototype.setValue = $.liger.controls.ComboBox.prototype.selectValue;
    //设置文本框和隐藏控件的值
    $.liger.controls.ComboBox.prototype.setInputValue = $.liger.controls.ComboBox.prototype._changeValue;
    $.liger.controls.ComboBox.prototype.unbindBodyClick = function () { $(document).unbind("click.combobox"); }
