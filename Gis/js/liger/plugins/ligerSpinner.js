...
(function ($)
{
    ...
    {
        return $.liger.run.call(this, "ligerSpinner", arguments);
    };
    $.fn.ligerGetSpinnerManager = function ()
    {
        return $.liger.run.call(this, "ligerGetSpinnerManager", arguments);
    };
    ...
    $.ligerDefaults.Spinner = {
        ...
        maxValue: null,         //最大值
        disabled: false,
        isHideArrow:false      //是否隐藏右侧箭头
    };

    $.ligerMethos.Spinner = {};

    $.liger.controls.Spinner = function (element, options)
    {
        $.liger.controls.Spinner.base.constructor.call(this, element, options);
    };
    $.liger.controls.Spinner.ligerExtend($.liger.controls.Input, {
        __getType: function ()
        {
            return 'Spinner';
        },
        ...
        _init: function ()
        {
            $.liger.controls.Spinner.base._init.call(this);
            var p = this.options;
            if (p.type == 'float')
            {
               // p.step = 0.1;
                var step = 0.1;
                if (p.decimalplace > 0)//如果有小数
                    step = "0.";
                for (var i = 0; i < p.decimalplace - 1; i++) {
                    step += "0";
                }
                step += "1";
                p.step = parseFloat(step);
        ...
        _render: function ()
        {
            ...
            //ie下限制输入中文
            g.inputText.css("ime-mode", "disabled");
            ...
            }).focus(function ()
            {
                ...
                var e = e || event;
                var k = e.keyCode;
                if ((k <= 57 && k >= 48) || (k >= 189 && k <= 190) || (k <= 105 && k >= 96) || (k == 8) || (k == 110)||(k==229))
                    return true;
                else {
                    return false;
                }
