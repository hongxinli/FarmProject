...
function windowloadByIndex(pageIndex) {
    var parm = getQueryString("page");
    var url = window.location.href;
    if (parm != null && parm.length > 0) {
        url = url.split("?")[0] + "?page=" + pageIndex;
    } else {
...
//树表格复选框，点击checkbox，把父级也打勾
function ckbValueObj(e) {
    //根据当前checkbox和兄弟级别checkbox的状态判断父级checkbox状态
    var ck = $("#" + e).attr("checked") == undefined ? $("#" + e).parents("li").siblings().find(":checkbox[checked]").length > 0 ? true : false : true;
    var item_id = '';
    var arry = new Array();
    arry = e.split('-');
    ...
    //勾选中父级checkbox
    if (item_id != "") {
        $("#" + item_id).attr("checked", ck);
        ckbValueObj(item_id);
    }
}
function ckbTableValueObj(e) {
    //根据当前checkbox和兄弟级别checkbox的状态判断父级checkbox状态
    var ck = $("#" + e).attr("checked") == undefined ? $("#" + e).parents("tr").siblings().find(":checkbox[checked]").length > 0 ? true : false : true;
    var item_id = '';
    var arry = new Array();
    arry = e.split('-');
    for (var i = 0; i < arry.length - 1; i++) {
...
//树表格复选框，点击checkbox，把子级也打勾
function ckbValueChildren(e) {
    var ck = $("#" + e).attr("checked") == undefined ? false : true;
    var _lis = $("#" + e).parents().siblings("ul").children("li");
