...
(function ($) {
    var l = $.liger;//不能删，有用
    l.IsJsonNull = function (p) {//检测是否为空json对象
        var plength = 0;//json属性个数
        for (var key in p) { plength++; }
        return plength == 0;
    };
    $.fn.ligerGrid = function (options) {
        return $.liger.run.call(this, "ligerGrid", arguments);
    };

    $.fn.ligerGetGridManager = function () {
        return $.liger.run.call(this, "ligerGetGridManager", arguments);
    };
    ...
    $.ligerDefaults.Grid = {
		...
		checkboxAll: true,                         //是否显示全选复选框
        allowHideColumn: true,                 //是否显示'切换列层'按钮
        prmarykeyColCss: 'Das-pkcol',                 //主键列背景样式
        notnullColCss: 'Das-nucol',                 //非空列背景样式
        ...
        scrollLoadCellsNum: 2000,				//滚动加载的单元格个数，超出此范围则滚动加载，保证载入效率
    ...
    $.ligerDefaults.Grid.formatters['float'] = function (value, column) {
        return 8;
    };
    //扩展currency类型的格式化函数
    ...
    $.ligerDefaults.Grid.formatters['date'] = function (value, column) {//debugger;
        ...
        if (typeof (value) == "string" && /^\/Date/.test(value)) {
            ...
            if (!(value instanceof Date)) {
                var d = value;
                value = new Date(value);//先试着转换下
                if (isNaN(value))//如果不行
                {
                    //首先处理成日期Date认识的格式,数据规范为yyyy-MM-dd hh24:mi:ss
                    //d=d.replace(/-/g,'-');
                    if (d.indexOf('-') != -1)//带-格式
                    {
                        var arrDate = d.split(' ');
                        var arrRq = arrDate[0].split('-');
                        var strTime = '';
                        if (arrDate.length > 1) {
                            var arrTime = arrDate[1].split(':');
                            strTime = ',' + arrTime[0] + ',' + arrTime[1] + ',' + arrTime[2];
                        }
                        var mth = parseInt(arrRq[1], 0);
                        if (mth >= 1)
                            mth -= 1;
                        eval('d=new Date(' + arrRq[0] + ',' + mth + ',' + arrRq[2] + strTime + ')');
    ...
    $.liger.controls.Grid = function (element, options) {
        $.liger.controls.Grid.base.constructor.call(this, element, options);
    };

    $.liger.controls.Grid.ligerExtend($.liger.core.UIComponent, {
        __getType: function () {
            return '$.liger.controls.Grid';
        },
        __idPrev: function () {
            return 'grid';
        },
        ...
        _init: function () {
            $.liger.controls.Grid.base._init.call(this);
            var g = this, p = this.options;
            //采集模式处理
            if (p.DasMode) {
                //基本属性控制
                p.enabledEdit = true;//启用编辑
                p.clickToEdit = true;//点击编辑
                p.rownumbers = true;//行序号
                p.alternatingRow = false;//取消隔行换色
                p.allowUnSelectRow = false;//不允许反选行
                p.title = null;//设置标题栏不显示
                p.mouseoverRowCssClass = '';//取消鼠标放上去效果
                p.allowHideColumn = false;//取消右键列头弹出层
                p.usePager = false;//取消分页
                p.async = false;//改成同步，否则有些元素获取的时候会为空
            }
            ...
            if (p.tree)//启用树型模式
            {
                ...
                p.tree.isParent = p.tree.isParent || function (rowData) {
                    //改为children必须有值才行---周峰
                    var exist = p.tree.childrenName in rowData && rowData[p.tree.childrenName] != null;
                    if (exist) {
                        if (l.IsJsonNull(rowData[p.tree.childrenName]))
                            exist = false;
                    }
        ...
        _render: function () {
            ...
            g.changedCells = {};
            g.deletedRows = [];
            g.deletedRowsForCJ = [];//扩展，采集录入使用--周峰
            g.editors = {};                 //多编辑器同时存在
        ...
        isHorizontalScrollShowed: function () {
            ...
            if (!inner.length || !g.gridheader.width()) return false;
            return g.gridheader.width() < g.gridheader[0].scrollWidth - 30;
        },
        _setHeight: function (h) {
            ...
            var arrH = g._calculateGridBodyHeight(h);
            var h = arrH[0];
            if (!isNaN(h)) {
                if (h > 0) {//周峰，解决复选框上下滚动条时 出现行水平不对齐的情况！
                    g.gridbody.height(h);
                    if (g.isHorizontalScrollShowed() && h > 17)
                        g.f.gridbody.height(h - 17);//要减去底部数据区水平滚动条的高度
                    else
                        g.f.gridbody.height(h);

                    g.gridview.height(h + arrH[1]);
        ...
        loadData: function (loadDataParm) {
            ...
            if (p.dataType == "local" || (p.dataAction == "local" && !loadServer)) {
                //加载进度条
                if (g.hasBind('loading')) g.trigger('loading');
                //else g.toggleLoading(true);
            }
        ...
        loadServerData: function (param, clause) {
            ...
            var ajaxOptions = {
                ...
                success: function (data) {
                    ...
                    g.data = data;
                    //if (p.dataAction == "server") {---周峰，服务器获取数据也要分页
                    //g.currentData = g.data;
                    //}
                    //else {
                    g.filteredData = g.data;
                    if (clause) g.filteredData[p.root] = g._searchData(g.filteredData[p.root], clause);//使用函数过滤
                    if (p.usePager)
                        g.currentData = g._getCurrentPageData(g.filteredData);
                    else
                        g.currentData = g.filteredData;
        ...
        _createEditor: function (editorBuilder, container, editParm, width, height) {
            var g = this, p = this.options;
            var editor = editorBuilder.create.call(g, container, editParm);

            if (editorBuilder.type == 'date' && p.formatters[editParm.column.type]&&editParm.column.format==p.dateFormat) //处理日期格式化数据
                editParm.value = p.formatters[editParm.column.type].call(g, editParm.value, editParm.column);

            if (editorBuilder.setValue)
                editorBuilder.setValue.call(g, editor, editParm.value, editParm);
            if (editorBuilder.setText && editParm.column.textField)
                editorBuilder.setText.call(g, editor, editParm.text, editParm);
            if (editorBuilder.resize)
                editorBuilder.resize.call(g, editor, width, height, editParm);

            var columnValidate = editParm.column.validate;
            editor.vo = editor.valueField || editor.inputText;//要验证的对象
            if (columnValidate) {
                if (typeof (columnValidate) == "string") {
                    editor.vo.attr("validate", columnValidate);
                }
                else if (typeof (editParm.column.validate) == "object") {
                    editor.vo.attr("validate", JSON2.stringify(columnValidate));
                }
                var rowdata = g.getRow(editParm.rowindex);
                var cellObj = g.getCellObj(rowdata, editParm.column);
                var id = g.id + "_editor_" + g.editorcounter + "_" + new Date().getTime();
                editor.vo.attr('name', id).attr('id', id);

                $(cellObj).attr('editor_oldid', function () { return $(this).attr('editor_id'); })
                //$(cellObj).attr('editor_id',editor.valueField.attr('data-ligerid'));//将编辑器的id存起来，始终是取最新的一个编辑器
                $(cellObj).attr('editor_id', id);//将编辑器的id存起来，始终是取最新的一个编辑器
            }

            if (editorBuilder.type == 'select')//下拉框去掉对body的点击，要不报脚本错误
            {
                editor.unbindBodyClick();
            }
        ...
        */
        beginEdit: function (rowParm, containerBulider) {
            ...
            for (var i = 0, l = g.columns.length; i < l; i++) {
                ...
                var editorControl = g._createEditor(editor, container, editParm, width-1, height);
        ...
        //某行是否带有复选框----周峰
        //{rowParm} rowindex或者rowdata
        //{bChecked} 判断是否是选中
        WithCheckbox: function (rowParm, bChecked) {
            var g = this;
            ...
            for (var i = 0; i < g.columns.length; i++) {
                var cellObj = g.getCellObj(rowdata, i);

                if ($(cellObj).hasClass('l-grid-row-cell-checkbox')) {
                    if (bChecked)//判断复选框是否选中
                        bChk = $(cellObj).parent().hasClass("l-selected");
                    else
                        bChk = true;
                    break;
                }
        ...
        deleteSelectedRow: function () {
            ...
            for (var i in g.selected) {
                ...
                if (o['__id'] in g.records)
                    g._deleteData.ligerDefer(this, 10, [o]);
            }

            g.reRender.ligerDefer(g, 20);
        },
        ...
        //移除某行的复选框-----周峰
        //{rowParm} rowindex或者rowdata
        removeChk: function (rowParm) {
            var g = this;
            if (!g.enabledCheckbox()) return;
            var rowdata = g.getRow(rowParm);
            for (var i = 0; i < g.columns.length; i++) {
                var cellObj = g.getCellObj(rowdata, i);
                if ($(cellObj).hasClass('l-grid-row-cell-checkbox')) {
                    $(cellObj).removeClass('l-grid-row-cell-checkbox');
                    return;
                }
            }
        },
        ...
        _deleteData: function (rowParm) {
            ...
            /*删除前先处理,保存初始值-----------周峰*/
            var o_id = $.extend({}, rowdata);//切记不能用=来操作，否则会指向同一个数据,得用extend
            var o = { "RowState": "", "RowData": {} };
            o.RowState = 'delete';
            g.formatRecord(o_id, true, true);
            $.extend(o.RowData, o_id);

            var delcol = [];//要删除的属性
            for (var _p in o.RowData) {
                var NewValue = o_id[_p];
                var column = g.getColumn(_p);

                if (_p != column.name)//限定属性在列范围内
                    delcol.push(_p);

                o.RowData[_p] = { "OldValue": g.getCellInitValue(o_id.__id, _p), "NewValue": NewValue };
            }
            for (var i in delcol) {
                delete o.RowData[delcol[i]];
            }
            delete o.RowData['__id'];
            g.deletedRows = g.deletedRows || [];
            g.deletedRowsForCJ.push(o);
            /**********************************************/
        ...
        */
        updateCell: function (arg, value, rowParm) {
            ...
            /*增加单元格改变处理*/
            var origindata = g.getCellInitValue(rowdata, column.name);
            var rowcell = g.getCellObj(rowdata, arg);
            var rowid = rowdata['__id'];
			var bChange=false;

			if(origindata==null&&value!="") bChange=true;
			else if (value != origindata&&origindata!=null) bChange=true;
			if (bChange) {//单元格状态改变应该和原始值对比
                if (!$(rowcell).hasClass('l-text-invalid-cell'))
                ...
                g.changedCells[rowid + "_" + column['__id']] = false;
            }
        ...
        _updateGridData: function () {//debugger;
            var g = this, p = this.options;
            g.recordNumber = 0;
            g.rows = [];
            g.records = {};
            ...
            function load(data, pid) {
                ...
                for (var i = 0, l = data.length; i < l; i++) {
                    var o = data[i];
                    //g.formatRecord(o);
                    g.formatRecord(o, false, false);//周峰改，保留行id
        ...
        addRow: function (rowdata, neardata, isBefore, parentRowData) {
            ...
            //聚焦到该行
            p.DasMode&&g.select(rowdata)&&g._setDasCss(rowdata);
            //g.getRowObj(rowdata).focus();
        ...
        //周峰， exi为特殊处理,
        getData: function (status, removeStatus, exi) {
            var g = this, p = this.options;
            var data = [];
            if (removeStatus == undefined) removeStatus = true;
            var jexi = { bnochk: false, delcols: [], bChecked: false };//是否包含复选框行数据，要删除的列,是否输出选中行
            $.extend(jexi, exi || []);
            for (var rowid in g.records) {
                var o = $.extend(true, {}, g.records[rowid]);
                $.each(jexi.delcols, function (i, v) { delete o[v]; })

                if (o[p.statusName] == status || status == undefined) {
                    if (exi == undefined || jexi.delcols.length)//获取全部数据或获取全部数据(删除某些列)
                        data.push(g.formatRecord(o, removeStatus));
                    else {
                        if (g.enabledCheckbox()) {
                            if (jexi.bnochk && g.WithCheckbox(rowid))//如果设置了不输出不包含复选框的行并且此行包含复选框
                                data.push(g.formatRecord(o, removeStatus));
                            else {
                                if (jexi.bChecked && g.WithCheckbox(rowid, true))//如果获取打勾数据
                                    data.push(g.formatRecord(o, removeStatus));
                            }
                        }
                    }
        ...
        //格式化数据
        formatRecord: function (o, removeStatus, removeId) {
            if (removeId == undefined) delete o['__id'];
            delete o['__previd'];
            delete o['__nextid'];
            delete o['__index'];
        ...
        updateDataStatus: function () {//保存完毕后更新状态为 nochanged
            var g = this, p = this.options;
            for (var rowid in g.records) {
                var o = g.records[rowid];
                o[p.statusName] = 'nochanged';
            }
            this.deletedRowsForCJ = [];//清空删除数据
            for (var cellid in g.changedCells) {
                var arrCellId = cellid.split('_');
                $(g.getCellObj(arrCellId[0], arrCellId[1])).removeClass("l-grid-row-cell-edited");
                g.changedCells[cellid] = false;
            }
        },
        _getbdData: function (status)//获取采集状态输出值
        {
            var g = this, p = this.options;
            var data = [];
            if (status != "delete") {
                ...
                for (var rowid in g.records) {
                    ...
                    if (o[p.statusName] == status)
                        d_change.push(g.formatRecord(o, true, false));
                }

                $.each(d_change, function (i, v) {
                    var o = { "RowState": "", "RowData": {} };
                    o.RowState = status;
                    $.extend(o.RowData, d_change[i]);
                    var delcol = [];//要删除的属性
                    for (var _p in o.RowData) {
                        var NewValue = d_change[i][_p];
                        var column = g.getColumn(_p);
                        if (_p != column.name && _p != "__id" && _p != "__id")//限定属性在列范围内
                            delcol.push(_p);

                        o.RowData[_p] = { "OldValue": g.getCellInitValue(d_change[i].__id, _p), "NewValue": NewValue }
                    }
                    delete o.RowData["__id"];
                ...
                data = this.deletedRowsForCJ;

            return data;
        },
        getDasData: function ()//---周峰，获取Das的数据
        {
            var g = this, p = this.options;
            var data = { "Cols": [], "Data": [] };
            for (var j in p.columns) {
                var o = { "colname": p.columns[j].name, "type": g.getColumnType(p.columns[j].name) };
                data.Cols.push(o);
            }
            data.Data = g.getChanges(true);
            return data;
        },
        checkPkNotNullData: function ()//---检查主键和非空列是否填写
        {
            var g = this, p = this.options;
            for (var i in g.rows) {
                for (var j in g.columns) {
                    if (g.columns[j].primarykey || g.columns[j].notnull) {
                        if ($.trim(g.rows[i][g.columns[j].columnname]) == '') return false;
                    }
                }
            }
        },
        checkPkRe: function ()//---检查主键是否重复
        {
            var g = this, p = this.options;
            var bPk = false;//是否有主键列
            var bPkRe = false;//是否有主键重复
            for (var j in g.columns) {
                if (g.columns[j].primarykey) {
                    bPk = true;
                    break;
                }
            }
            if (bPk) {
                var gridrows = [];
                $.each(g.rows, function (i, row) {
                    var d = {};
                    $.each(g.columns, function (j, column) {
                        if (column.primarykey) {
                            if (column.type == 'number') {
                                if (column.datalength && column.datalength.toString().indexOf('.') != -1)//float型
                                    d[column.columnname] = parseFloat(row[column.columnname], 10);//提取键值
                                else
                                    d[column.columnname] = parseInt(row[column.columnname], 10);//提取键值
                            }
                            else
                                d[column.columnname] = row[column.columnname];//提取键值
                        }
                    })
                    gridrows.push(d);
                })
                ...
                if (primarykeyDataArr.length < g.rows.length)//唯一的长度小于原来长度
                    bPkRe = true;
            }

            return bPkRe;
        },
        getChanges: function (das) {//das为返回采集可视化定制的格式，包含原始状态值
            var g = this, p = this.options;
            var data = [];
            if (das) {
                //获取全部状态数据
                $.merge(data, g._getbdData('add'));
                $.merge(data, g._getbdData('update'));
                $.merge(data, g._getbdData('delete'));
                $.merge(data, g._getbdData('nochanged'));
            }
            else {
                if (this.deletedRows) {
        ...
        getColumn: function (columnParm) {
            var g = this, p = this.options;
            if (typeof columnParm == "string") // column id或者列名
            {
                if (g._isColumnId(columnParm))
                    ...
                    for (i = 0; i < g.columns.length; i++) {
                        if (g.columns[i].id == columnParm || g.columns[i].name == columnParm) {
                            return g.columns[i];
                        }
                    }
                    //return g.columns[parseInt(columnParm)];---周峰去掉
                }
            }
        ...
        //改变排序
        changeSort: function (columnName, sortOrder) {//debugger;
            var g = this, p = this.options;//alert("p.dataAction:"+p.dataAction+" p.dataType:"+p.dataType)
            if (g.loading) return true;

            //if (p.dataAction == "local") {
            //if (p.dataAction == "local"||(p.dataAction == "server"&&p.dataType == "local")) {
        ...
        getSelectedRows: function () {
            var g = this, p = this.options;
            var arr = [];
            if (g.scrollLoad)//滚动加载 
            {
                var bSelect = false;
                if (g.checkboxallClicked)//点击过全选复选框，以目前状态为准
                {
                    bSelect = $(".l-grid-hd-row").hasClass("l-checked");
                    if (bSelect) return g.rows;
                    else {
                        for (var i in g.rows) { 
							if (g.isSelected(g.rows[i]))
								arr.push(g.rows[i]);
						}
						return arr;
                    }
                    ...
                    for (var i in g.rows) {
                        if (p.isSelected && p.isSelected(g.rows[i]) && g.getRowObj(g.rows[i]) == null&&$.inArray(g.rows[i], arr)==-1)//未点击过，是初始化,并且这时候还没加载出来
                            arr.push(g.rows[i]);
                    }
                    return arr;
                }
        ...
        //判断某单元格是否可编辑，在对应列已设置了编辑器的情况下---周峰
        _CellEnableEdit: function (CellObj) {
            var g = this, p = this.options;
            return $(CellObj).attr("enabledEdit") == "false" || g._getCellColumn(CellObj.id).issystem ? false : true
        },
        //删除某行编辑器---周峰
        removeEditor: function (rowdbdata) {
            if (typeof (rowdbdata) == "object") {
                var g = this;
                for (var i in g.columns) {
                    var cellcbObj = g.getCellObj(rowdbdata, i);
                    $(cellcbObj).attr("enabledEdit", false);
                }
            }
        },
        //改变列表头内容
        ...
        _initColumns: function () {
            ...
            function setColumn(column, level, pid, previd) {
                ...
                column['_mergecell'] = column.mergecell;//周峰，添加是否是合并其下的所有子单元格
                return leafcount;
            }
            var lastid = -1;
            //行序号
        ...
        _getColumnWidth: function (column) {
            ...
            var colwidth = column.width || p.columnWidth || column.minWidth || column.maxWidth;

            if (!colwidth || colwidth == "auto") {
                var autoColumnNumber = 0, noAutoColumnWidth = 0;
                $(g.columns).each(function (i, col) {
                    var colwidth = col.width || p.columnWidth || col.minWidth || col.maxWidth;
                ...
                /*处理自动分配宽度的情况---周峰*/
                var lwidth = 18;//去除右侧滚动条的宽度
                if (g.enabledCheckbox() && p.rownumbers)
                    lwidth = -33;
                else if (g.enabledCheckbox() || p.rownumbers)
                    lwidth = -10;

                if (g.enabledCheckbox()) lwidth += p.checkboxColWidth;
                if (g.enabledDetail()) lwidth += p.detailColWidth;
                if (p.rownumbers) lwidth += p.rownumbersColWidth;

                colwidth = parseFloat((g.grid.width() - noAutoColumnWidth - lwidth) / autoColumnNumber) - 1;
                if (colwidth <= 0) colwidth = p.minColumnWidth;
            }
        ...
        _createHeaderCell: function (column) {
            ...
            if (column.display && column.display != "") {
                ...
                if (column.unit && column.unit != "")
                    headerText += '(' + column.unit + ')';
            }
            else if (column.headerRender)
                headerText = column.headerRender(column);
            else
                headerText = "&nbsp;";
            var jcell_hd = $(".l-grid-hd-cell-text:first", jcell);
            jcell_hd.html(headerText);

            var colinfo = "";
            if (column.primarykey) {
                jcell_hd.addClass('Das-pkcol-header');//主键列头样式
                colinfo = "主键列 ";
            }
            else if (column.notnull) {
                jcell_hd.addClass('Das-nucol-header');//非空列头样式
                colinfo = "非空列 ";
            }

            //增加表头提示
            if (p.DasMode && !column.issystem) {
                var header = column.display;
                var strLen = "";
                if (column.datalength) {
                    var d = Das.parsePrecision(column.datalength);
                    if (d.type == 'int')
                        strLen = " 宽度:" + column.datalength;
                    else {
                        strLen = " 宽度:" + d.tsw;
                        strLen += " 小数位:" + d.xsw;
                    }
                }
                colinfo += "名称:[" + header + "] 数据类型:(" + column.type.toUpperCase() + strLen + ")";

                //增加悬浮提示
                jcell.removeAttr("title").ligerHideTip();
                jcell.attr("title", colinfo).ligerTip({
                    distanceX: 5,
                    distanceY: -3,
                    auto: true
                });
        ...
        _initBuildGridHeader: function () {
            ...
            //如果不允许排序，则表头手型去掉
            if (!p.enabledSort) {
				$(".l-grid-hd-cell-text,.l-grid-row-cell-checkbox span",g.grid).css('cursor','default');
				$(".l-grid-hd-cell-checkbox .l-grid-hd-cell-inner div",g.grid).css('cursor','pointer');
            }

			if(p.checkbox&&!p.checkboxAll)//如果有全选
			{
				$(".l-grid-hd-cell-checkbox .l-grid-hd-cell-text", g.grid).css('cursor', 'default');
			}
            //根据表头行高动态控制line-height----周峰
            g.gridheader.add(g.f.gridheader).css('line-height', p.headerRowHeight + "px");

            //再单独根据表头行的数据是否包含br标签，单独设定某个单元格的line-height
            var GridHeaderMaxBrCount = 0;//表头行内部换行的最大个数
            $("td", g.f.gridheader).add($("td", g.gridheader)).each(function (i) {
                var otd = $(this);
                var oldhtml = otd.html();
                if (oldhtml != null) {
                    var newhtml = oldhtml.replace(/<br>/g, "<br/>").replace(/<BR>/g, "<br/>");
                    var BrCount = newhtml.split('<br/>').length;
                    if (BrCount > GridHeaderMaxBrCount)
                        GridHeaderMaxBrCount = BrCount;

                    if (BrCount > 1)
                        otd.css('line-height', p.headerRowHeight / BrCount + "px");

                    if (i < g.columns.length && g.columns[i].ischeckbox)//复选框列
                    {
                        $(".l-grid-hd-cell-inner div", otd).each(function () {
                            $(this).css('margin-top', function () {
                                return ($(this).parent("div").height() - $(this).height()) / 2 + "px";//css用函数设置的时候this,不会接收到！！attr(),each()都可以
                            })

                        });
                        //alert(otd)[0].outerHTML();
                    }
                }
            })
        },
        ...
        _initFootbar: function () {
            ...
            if (p.usePager) {
                ...
                if (!p.tree) {
                    ...
                    $('.l-bar-message', g.toolbar).siblings().not('.l-clear').remove();//除了自定义的工具栏信息，分页信息去除------周峰
            }
            else {
                g.toolbar.hide();
            }
        ...
        _fillGridBody: function (data, frozen) {
            var g = this, p = this.options;
			//加载数据 
            var bodyinner = $("<div class='l-grid-body-inner' />");
            (frozen ? g.f.gridbody : g.gridbody).empty().append(bodyinner);
            var bodytable = $("<table class='l-grid-body-table' cellpadding='0' cellspacing='0'><tbody></tbody></table>");
            bodyinner.append(bodytable);

			var _vscrollEl = $("<div class='l-grid-vscroll l-scroll' style='display: block;overflow-y:scroll;overflow-x:hidden;right:0;bottom:0;position:absolute;width:18px;'><div class='l-grid-vscroll-content' style='width:1px;overflow:hidden;left:0;top:0;position:absolute'></div></div>");

			if(g.scrollLoad)
			{
				if (g.gridbody.parent().find(".l-grid-vscroll").length == 0)
					g.gridbody.parent().append(_vscrollEl.height(g.gridbody.height()));

				g._vscrollEl=g.gridbody.parent().find(".l-grid-vscroll");
			}
			else
				g._vscrollEl&&g._vscrollEl.remove();
            ...
            if (g.enabledGroup()) //启用分组模式
            {
                ...
                $(groupsdata).each(function (i, item) {
                    var newrow = $("<tr/>");
					newrow.addClass('l-grid-grouprow');

					if (groupsdata.length == 1)
						newrow.addClass('l-grid-grouprow-last l-grid-grouprow-first');
					else if (i == groupsdata.length - 1)//周峰改,原来没有else ,会多输出tr,造成标签不闭合
						newrow.addClass('l-grid-grouprow-last');
					else if (i == 0)
						newrow.addClass('l-grid-grouprow-first');

					newrow.attr('groupindex', i);

					var newtd = $('<td colSpan="' + g.columns.length + '" class="l-grid-grouprow-cell" />');
					newtd.append('<span class="l-grid-group-togglebtn">&nbsp;&nbsp;&nbsp;&nbsp;</span>');
					if (p.groupRender)
						newtd.append(p.groupRender(groups[i], item, p.groupColumnDisplay));
					else
						newtd.append(p.groupColumnDisplay + ':' + groups[i]);

					newrow.append(newtd);

					g._creatRowsFromData(item, frozen,bodytable);

					//汇总
					if (g.isTotalSummary())
						bodytable.append(g._getTotalSummaryHtml(item, "l-grid-totalsummary-group", frozen));
                });
            }
            else 
				g._creatRowsFromData(data, frozen,bodytable);
        ...
        _getHtmlFromData: function (data, frozen) {
            ...
            for (var i = 0, l = data.length; i < l; i++) {
                ...
                $(g.columns).each(function (columnindex, column) {
                    ...
                    var tdhtmlarr = [];
                    tdhtmlarr.push('<td');
                    tdhtmlarr.push(' id="' + g._getCellDomId(item, this) + '"');
                    //如果是行序号(系统列)
                    if (this.isrownumber) {
                        tdhtmlarr.push(' class="l-grid-row-cell l-grid-row-cell-rownumbers" style="width:' + this.width + 'px"><div class="l-grid-row-cell-inner"');
                        if (p.fixedCellHeight)
                            tdhtmlarr.push(' style = "height:' + p.rowHeight + 'px;" ');
                        else
                            tdhtmlarr.push(' style = "min-height:' + p.rowHeight + 'px;" ');

                        tdhtmlarr.push('>' + (p.DasMode ? '' : (parseInt(item['__index']) + 1)) + '</div></td>');
                    }
                        //如果是复选框(系统列)
                    else if (this.ischeckbox) {
                        tdhtmlarr.push(' class="l-grid-row-cell l-grid-row-cell-checkbox" style="width:' + this.width + 'px"><div class="l-grid-row-cell-inner"');
                        if (p.fixedCellHeight)
                            tdhtmlarr.push(' style = "height:' + p.rowHeight + 'px;" ');
                        else
                            tdhtmlarr.push(' style = "min-height:' + p.rowHeight + 'px;" ');
                        tdhtmlarr.push('>');
                        tdhtmlarr.push('<span class="l-grid-row-cell-btn-checkbox"></span>');
                        tdhtmlarr.push('</div></td>');
                    }
                        //如果是明细列(系统列)
                    else if (this.isdetail) {
                        tdhtmlarr.push(' class="l-grid-row-cell l-grid-row-cell-detail" style="width:' + this.width + 'px"><div class="l-grid-row-cell-inner"');
                        if (p.fixedCellHeight)
                            tdhtmlarr.push(' style = "height:' + p.rowHeight + 'px;" ');
                        else
                            tdhtmlarr.push(' style = "min-height:' + p.rowHeight + 'px;" ');
                        ...
                        if (!p.isShowDetailToggle || p.isShowDetailToggle(item)) {
                            tdhtmlarr.push('<span class="l-grid-row-cell-detailbtn"></span>');
                        }
                        tdhtmlarr.push('</div></td>');
                    }
                    else {
                        var colwidth = this._width;
                        tdhtmlarr.push(' class="l-grid-row-cell ');
                        if (g.changedCells[rowid + "_" + this['__id']]) tdhtmlarr.push("l-grid-row-cell-edited ");
                        if (this.islast)
                            tdhtmlarr.push('l-grid-row-cell-last ');
                        tdhtmlarr.push('"');
                        //if (this.columnname) gridhtmlarr.push('columnname="' + this.columnname + '"');
                        tdhtmlarr.push(' style = "');
                        tdhtmlarr.push('width:' + colwidth + 'px; ');
                        if (column._hide) {
                            tdhtmlarr.push('display:none;');
                        }
                        tdhtmlarr.push(' ">');
                        tdhtmlarr.push(g._getCellHtml(item, column));
                        tdhtmlarr.push('</td>');
                    }
					gridhtmlarr.push(tdhtmlarr.join(''));
                });
                gridhtmlarr.push('</tr>');
            }
            return gridhtmlarr.join('');
        },
		_addInitValue:function (data) {//给单元格添加初始值
			var g = this, p = this.options;
			for (var i = 0, l = data.length; i < l; i++) {
                var item = data[i];
				$(g.columns).each(function (columnindex, column) {
					var InitValue = g._getValueByName(item, column.name);
					if (column.type == 'date' && p.formatters[column.type])//处理日期格式
						InitValue = p.formatters[column.type].call(g, InitValue, column);
					var cellObj =g.getCellObj(item,column);
					$(cellObj).attr("InitValue", InitValue);//周峰，添加上初始值属性
				})
			}
		},
        _showData: function () {
            ...
            if (!data || !data.length) {
                ...
                $("<div></div>").addClass("l-grid-body-inner").appendTo(g.gridbody).css({
                    width: g.gridheader.find(">div:first").width()-50,
                    height: g.gridbody.height()-50
                });
            ...
            //先判断是否开启滚动加载
			if (g.rows.length <= g._getScrollLoadRowLength()||p.DasMode)//如果是数据量少或者Das采集模式正常加载
				g.scrollLoad = false;
			else
				g.scrollLoad = true;
            ...
            var lazyload = function () {
                ...
                g.gridbody[0].scrollTop = 0;
            }
            lazyload.ligerDefer(g, 1, []);

            //隐藏进度条
            g.toggleLoading.ligerDefer(g, 10, [false]);
        },
		_setRowHoverCss: function (rows){//设置行悬浮样式
			var g = this, p = this.options;
			if(!rows) rows=g.rows;
			if (p.mouseoverRowCssClass) {
				for (var i in rows) {
					var rowobj = $(g.getRowObj(rows[i]));
					if (g.enabledFrozen())
						rowobj = rowobj.add(g.getRowObj(rows[i], true));
        ...
        _setDasCss: function (rowdata) {//设置主键列和非空列的样式
            var g = this, p = this.options;
            if (!rowdata) {
                ...
                $(g.columns).each(function (j, col) {
                    var cellObj = g.getCellObj(rowdata, col);

                    if (col.primarykey)//主键
                        $(cellObj).addClass(p.prmarykeyColCss);
                    else if (col.notnull)//非空
                        $(cellObj).addClass(p.notnullColCss);
                })
            }
        },
        _fixHeight: function () {
        ...
        _getCellColumn: function (cellid) {//周峰，根据单元格id获取对应列
            var columnid = cellid.split('|')[cellid.split('|').length - 1];
            return this.getColumn(columnid);
        },
        _getCellRowObj: function (cellid) {//周峰，根据单元格id获取对应行
            var rowid = cellid.split('|')[2];
            return this.getRowObj(rowid);
        },
        getEditor: function (cellid) {//周峰，根据单元格id获取编辑器
            var rowid = cellid.split('|')[2];
            return this.getRowObj(rowid);
        },
        getCellInitValue: function (rowid, colname)//获取单元格初始值
        {//debugger;
            var rowdata = this.getRow(rowid);
            var column = this.getColumn(colname);
            var cellObj = this.getCellObj(rowdata, column);
            var InitValue = $(cellObj).attr("InitValue");
            return InitValue != null ? InitValue : null;
        },
        _getCellNextOrPre: function (cellobj, type)//获取某单元格当前行的下/上个单元格，区分是冻结列和非冻结列，不能用jq的next()方法，因为不是一个行对象
        {
            var g = this, p = this.options;
            var columnindex = g._getCellColumn(cellobj.id).columnindex;
            if (type == 'next') {
                if (columnindex < g.columns.length - 1)//不超过最后1列
                    return $(g.getCellObj(g._getCellRowObj(cellobj.id), g.getColumn(g._getCellColumn(cellobj.id).columnindex + 1)));
                else return null;
            }
            else if (type == 'prev') {
                if (columnindex > 0)//不超过第1列
                    return $(g.getCellObj(g._getCellRowObj(cellobj.id), g.getColumn(g._getCellColumn(cellobj.id).columnindex - 1)));
                else return null;
            }
        },
        _getRowNextOrPre: function (rowobj, type)//获取某行的下/上一行，不能用jq的next()方法，因为不是一个行对象
        {
            var g = this, p = this.options;
            if (type == 'next')
                return g.getRowObj(g.getRow(rowobj).__nextid, true);
            else if (type == 'prev')
                return g.getRowObj(g.getRow(rowobj).__previd, false);
        },
        getCellNextOrPre: function (cellobj, type) {
            var g = this, p = this.options;
            var curRowObj = g._getCellRowObj(cellobj.id);
            if (type == "next") {
                var curRowNextCell = g._getCellNextOrPre(cellobj, type);
                //var curRowNextCell=$(cellobj).next();
                while (curRowNextCell && !g._CellEnableEdit(curRowNextCell[0]))//如果当前行下1单元格并且可编辑
                {
                    curRowNextCell = g._getCellNextOrPre(curRowNextCell[0], type);
                }
                if (curRowNextCell)
                    return curRowNextCell;
                else//移动到下一行
                {
                    var nextRowObj = g._getRowNextOrPre(curRowObj, type);
                    if (nextRowObj) {
                        g.select(nextRowObj);//选中下一行
                        var firstcell = $(nextRowObj).find(">td:first");
                        while (firstcell.length && !g._CellEnableEdit(firstcell[0])) {
                            firstcell = g._getCellNextOrPre(firstcell[0], type);
                        }
                        return firstcell;
                    }
                    else
                ...
                var curRowPreCell = g._getCellNextOrPre(cellobj, type);
                while (curRowPreCell && !g._CellEnableEdit(curRowPreCell[0]))//如果当前行上1单元格并且可编辑
                {
                    curRowPreCell = g._getCellNextOrPre(curRowPreCell[0], type);
                }
                if (curRowPreCell)
                    return curRowPreCell;
                else//移动到上一行
                {
                    var preRowObj = g._getRowNextOrPre(curRowObj, type);
                    if (preRowObj) {
                        g.select(preRowObj);//选中上一行
                        var endcell = $(preRowObj).find(">td:last");
                        while (endcell.length && !g._CellEnableEdit(endcell[0])) {
                            endcell = g._getCellNextOrPre(endcell[0], type);
                        }
                        return endcell;
                    }
                    else
        ...
        getRowCellWidthKey: function (cellobj, type)//获取某单元格的"下一行/上一行"对应单元格
        {//debugger;
            var g = this, p = this.options;
            var curRowObj = $(this._getCellRowObj(cellobj.id));
            var colIndex = curRowObj.find("td").index($(cellobj));
            switch (type) {
                case "down"://下移
                    var nextRowObj = curRowObj.next();
                    if (nextRowObj.length) {
                        g.select(g.getRow(nextRowObj[0]));//选中下一行
                        var firstcell = nextRowObj.find(">td:eq(" + colIndex + ")");
                        return firstcell;
                    }
                    else
                        return null;
                    break;
                case "up"://上移
                    var prevRowObj = curRowObj.prev();
                    if (prevRowObj.length) {
                        g.select(g.getRow(prevRowObj[0]));//选中上一行
                        var firstcell = prevRowObj.find(">td:eq(" + colIndex + ")");
                        return firstcell;
                    }
                    else
                        return null;
        ...
        //新修改，改变之前的方法
        _scrollLoadRows: function (data, frozen) {
            if (!data) return "";
            var g = this, p = this.options;
            var gridhtmlarr = [];
            var rows = [];
            ...
            (frozen ? g.f.gridbody : g.gridbody).find(".l-grid-body-table").html(gridhtmlarr.join(''));

            //目的就是让可视化能编辑
            g.gridbody.next(".l-grid-vscroll").width(18);

            return rows;
        },
        _getScrollLoadRowLength: function (rowcount) {
            var g = this, p = this.options;
            return parseInt(p.scrollLoadCellsNum / g.columns.length);
        },
        _creatRowsFromData: function (data, frozen,container) { 
            if (!data) return "";
            var g = this, p = this.options;

            if (!g.scrollLoad)
			{
				container.html(g._getHtmlFromData(data,frozen));
				g.gridbody.css("overflow-y", "auto");
				g._addInitValue(data);
			}
            else 
			{
                if (g.isHorizontalScrollShowed())
					g.fullScreenRowLen = Math.floor((g.gridBodyHeight-17)/ p.rowHeight);//设置全屏展示的行数
				else
					g.fullScreenRowLen = Math.floor(g.gridBodyHeight/ p.rowHeight);//设置全屏展示的行数

				g._vscrollEl[0].scrollTop=0;
				$(".l-grid-vscroll-content", g._vscrollEl).height(data.length * p.rowHeight);//先计算整体高度
                g._scrollLoadRows(data.slice(0, g.fullScreenRowLen),frozen);//先加载满屏数据

				var loadstep = g.fullScreenRowLen;//滚动加载的步长
			
                if (g.fullScreenRowLen < data.length) {
                    g.gridbody.css("overflow-y", "scroll");//必须设置scroll占据位置
					var _scrollRecord;//记录当前的起始记录
                    //表体 - 滚动加载事件 
                    g._vscrollEl.unbind('scroll.gridlazy').attr('st',function(){return this.scrollTop;}).bind('scroll.gridlazy',function (e) {
                        var o=this;
						var st=o.scrollTop,orist=parseInt($(o).attr('st'),10);
						$(this).attr('st',st);
						if(st!=orist)//保证是竖向滚动
						{
							var _offsetHeight = this.offsetHeight;
							var _scrollTop = st;
							var _scrollHeight = this.scrollHeight;

							g.gridbody[0].scrollTop=st%p.rowHeight;//模拟真实的滚动效果
							...
							this._scrollTopTimer = setTimeout(function () {
								...
								var scrollRecord = Math.ceil(_scrollTop/loadstep);
								//alert(_scrollRecord+" "+scrollRecord)
								if(scrollRecord==_scrollRecord){alert(1); return ;}//和原来起始记录一样
								if ((_scrollHeight - _scrollTop) <= _offsetHeight)
									scrollRecord = data.length - loadstep;
								if (scrollRecord > data.length)
									scrollRecord = data.length - loadstep;

								_scrollRecord=scrollRecord;//记录当前的起始记录
								
								if (g.enabledFrozen())
									g._scrollLoadRows(data.slice(scrollRecord, scrollRecord + loadstep), true);

								var rows = g._scrollLoadRows(data.slice(scrollRecord, scrollRecord + loadstep), false);
								g._setRowHoverCss(rows);//设置行悬浮样式

								if ($(".l-grid-hd-row").hasClass("l-checked")) {
									var bSelect = false;
									if (g.checkboxallClicked)//点击过复选框，以目前状态为准
									{
										bSelect = $(".l-grid-hd-row").hasClass("l-checked");
										for (var i in rows) {
											if (g.WithCheckbox(rows[i]))//如果此行带有复选框
											{
												if (bSelect)
													g.select(rows[i]);
												else
										...
										for (var i in rows) {
											if (p.isSelected && p.isSelected(rows[i]))//未点击过，是初始化
												bSelect = true;
											else
												bSelect = false;
											if (g.WithCheckbox(rows[i]))//如果此行带有复选框
								...
								g.toggleLoading.ligerDefer(g, 50, [false]);
                    ...
                    g.gridbody.bind("mousewheel", function () {
                      //支持目前所有显示设备的最高分辨率
                        g.gridbody.next(".l-grid-vscroll").width(3000);
                    })
                    if ($.browser.mozilla) {
                        g.gridbody.bind("DOMMouseScroll", function () {
                           g.gridbody.next(".l-grid-vscroll").width(3000);
                        })
                    }
                }
        ...
        _getCellContent: function (rowdata, column) {
            ...
            if (column.isrownumber) return p.DasMode ? '&nbsp;' : parseInt(rowdata['__index']) + 1;
            var rowid = rowdata['__id'];
            ...
            //周峰，支持多列树形递进模式
            if (p.tree) {
                var o = p.tree.columnName != null ? p.tree.columnName : p.tree.columnId;
                var field = p.tree.columnName != null ? column.name : column.id;
                if (o != null) {
                    var arrcolumnName = [];
                    if (typeof o == 'object')//数组形式，多列
                        arrcolumnName = o;
                    else
                        arrcolumnName.push(o);//单列模式

                    var cindex = $.inArray(field, arrcolumnName);
                    if (cindex != -1)//如果在列数组中
                        content = g._getTreeCellHtml(content, rowdata, cindex, arrcolumnName.length == 1);
                }
            }
        ...
        _getTreeCellHtml: function (oldContent, rowdata, cindex, singlecol) {
            ...
            {
                if (isParent && level == (cindex + 1))//如果父节点并且匹配层次
                {
                    if (isExtend)
                        content += "<div class='l-grid-tree-space l-grid-tree-link l-grid-tree-link-open'></div>";
                    else
        ...
        _applyEditor: function (obj) {//单个单元格编辑
            var g = this, p = this.options;
            ...
            if (column.editor.type && p.editors[column.editor.type]) {
                ...
                if (column.editor.type == 'date'&&column.format==p.dateFormat)//是标准的日期格式需特殊处理
                    currentdata = p.formatters[column.editor.type].call(g, currentdata, column);
                ...
                var editor = p.editors[column.editor.type],
                    ...
                    width = $(rowcell).width() - 1, height = $(rowcell).height(),
                    ...
                    left = (column.frozen ? pc.left : pc.left + pb.left + pv.left),//修复锁定列下出现的位置偏差
                ...
                container
                    .css({ left: left + 1, top: $.browser.safari ? top + 2 : (column.editor.type == 'textarea' ? top : top + 1) })
                    .show();

                //首先隐藏冒泡提示，有些情况下不自动隐藏
                if (p.DasMode)
                    $(rowcell).removeAttr("title").ligerHideTip();

                if (column.textField) editParm.text = g._getValueByName(rowdata, column.textField);

                var editorInput = g._createEditor(editor, container, editParm, width, height - 1);//创建编辑器
                editorInput.inputText&&editorInput.inputText.click().focus();//聚焦编辑器

                g.editor = { editing: true, editor: editor, input: editorInput, editParm: editParm, container: container };

                g.unbind('sysEndEdit');
                g.bind('sysEndEdit', function () {
                    var newValue = editor.getValue(editorInput, editParm);//这个地方获取的是字符串

                    if (column.editor.type == 'date'&&column.format==p.dateFormat)//日期特殊处理
                        newValue = p.formatters[column.editor.type].call(g, newValue, column);
                    //if (newValue!= currentdata) {//不应该和现在值对比，应该和原始值对比
                    var origindata = g.getCellInitValue(rowdata, columnname);
					
                    var bChange=false;
					if(origindata==null&&newValue!="") bChange=true;
					else if (newValue != origindata&&origindata!=null) bChange=true;
					if (bChange) {//单元格状态改变应该和原始值对比
                        if (!$(rowcell).hasClass('l-text-invalid-cell'))
                            $(rowcell).addClass("l-grid-row-cell-edited");
                        g.changedCells[rowid + "_" + column['__id']] = true;
        ...
        //可实现合并单元格,可以全部合并，也可以指定列名合并，合并的单元格在同一组内，从前往后，即横向分组形式----------周峰
        //arrColumnName：列名数组，如['a','b']
        mergeCell: function (arrColumnName) {
            var g = this, startCell, targetCell;
            //合并单元格后禁止排序和显示列头
            this.options.enabledSort = false;
            //alert(g.options.enabledSort)
            if (arrColumnName.length)
                $.each(arrColumnName, function (i, v) { arrColumnName[i] = v.toLowerCase(); })//将数组元素转换成小写

            for (var j = g.columns.length - 1; j >= 0; j--) {//debugger;
                var cn = g.columns[j].columnname;
                if (!cn) return;
                if (arrColumnName == undefined || $.inArray(cn.toLowerCase(), arrColumnName) != -1)//传递了数组或者传递了，列名并且在里面
                {//debugger;
                    for (var i = g.recordNumber - 1; i >= 1; i--) {
                        startCell = $(g.getCellObj(this.getRow(i), j));
                        targetCell = $(g.getCellObj(this.getRow(i - 1), j));
                        var bGroup = false;
                        if (j >= 1) {
                            var pcn = g.columns[j - 1].columnname;
                            if (!pcn) bGroup = true;//说明是除了序号列和复选框列的第1个数据列
                                //判断是否在一个分组内,往左找第1个不一样的
                            else {
                                var c = 0;
                                var sp = startCell.prevAll(), tp = targetCell.prevAll(), _c = sp.length;
                                $.each(sp, function (_i, t) {//debugger;
                                    if (!g.columns[sp.length - _i].columnname) _c--;
                                    if ($(t).text() == tp.eq(_i).text() && g.columns[sp.length - _i].columnname && $(t).text() != "")
                                        c++;
                                })
                                bGroup = (c == _c);//说明往前都是一样的
                            }
                        }
                        else
                            bGroup = true;//说明是第1个数据列

                        if (bGroup && startCell.text() == targetCell.text() && targetCell.text() != "") {
        ...
        _getTotalCellContent: function (column, data) {
            ...
            if (column.totalSummary) {
                ...
                if (column.totalSummary.render) {
                    ...
                    if (isExist('sum'))
                        totalsummaryArr.push("<div>合计：" + sum + "</div>");
                    if (isExist('count'))
        ...
        _getSrcElementByEvent: function (e) {
            ...
            if (jobjs.index(this.element) == -1) {
                if (g._isEditing(jobjs)) return { editing: true };//grid内部
            ...
            if (r.gridheader) {
                ...
                r.checkboxall = fn("l-grid-hd-cell-btn-checkbox");//周峰，原来用的是td，精确说是那个复选框div才对
                if (r.hcell) {
        ...
        _setEvent: function () {
            ...
            g.grid.bind("keydown.grid", function (e) {//增加键盘事件----周峰
        ...
        _onKeyDown: function (e) {//--------周峰，增加TAB、回车、上下和左右箭头操控,只针对可编辑单元格
            var g = this, p = this.options;
            var src = g._getSrcElementByEvent(e);

            if (e.which != "9") return;//先支持TAB

            if ((e.which == "9" || e.which == "39")) {//TAB键或者是方向右键并且是编辑状态
                e.preventDefault();//阻止默认行为
                if (src.editing) {
                    if (g.checkCellLeave() == false) return;
                    g.editor.input.inputText.trigger('change');//TAB是不引发input的change，必须手动触发
                    g.endEdit();
                    var targetcell = g.getCellNextOrPre(g.curEditCell, 'next');
                    if (targetcell) {
                        //首先判断单元格位置是否在可视范围内
                        var pc = $(targetcell).position(), width = $(targetcell).width();
                        if (pc.left + width > g.gridview2.width())//pc.left为相对于锁定div的位置
                            g.gridbody[0].scrollLeft = pc.left + width - g.gridview2.width();

                        targetcell.click();
                        if (g.editor.input)//debugger;
                            g.editor.input.inputText.trigger('click').focus()[0].select();
                    }
                }
                else if (e.which == "9")//如果有选择的一行，则进入第1个可以编辑的单元格
                {
                    var rowobj = g.getSelectedRowObj();
                    if (rowobj) {
                        var firstcellobj = $(rowobj).find('>td:first');

                        while (firstcellobj && !g._CellEnableEdit(firstcellobj[0]))//如果当前行下1单元格并且可编辑
                        {
                            firstcellobj = g._getCellNextOrPre(firstcellobj[0], 'next');
                        }
                        if (firstcellobj)
                            firstcellobj.trigger('click');
                    }
                }
            }
            else if (e.which == "37" && src.editing) {//方向左键
                var targetcell = g.getCellNextOrPre(g.curEditCell, 'prev');
                if (targetcell) {
                    g.endEdit();
                    targetcell.click();
                }
            }
            else if (e.which == "13" && src.editing) {//回车并且是编辑状态
                //e.preventDefault();//阻止默认行为
                g.endEdit();
                /*var targetcell=g.getRowCellWidthKey(g.curEditCell,'down');
				if(targetcell)
				{
					targetcell.click();
					if(g.editor.input)//debugger;
                ...
                if (targetcell) {
                    targetcell.click();
                    if (g.editor.input)//debugger;
                        g.editor.input.inputText.trigger('click').focus();
                }
            }
            else if (e.which == "40") {//下移键
                e.preventDefault();//阻止默认行为
                e.stopPropagation();//阻止冒泡
                if (src.editing)//如果是正在编辑的单元格
                {
                    var targetcell = g.getRowCellWidthKey(g.curEditCell, 'down');
                    if (targetcell) {
                        g.endEdit();
                ...
                {
                    g._UpDownSelect('down');
                }
            }
            else if (e.which == "35") {//end键
            ...
            else if (e.which == "36") {//home键
                e.preventDefault();//阻止默认行为
                e.stopPropagation();//阻止冒泡
                var tips = g.goTo('first');
                if (tips != '') alert(tips);
            }
            else if (e.which == "38") {//上移键
                e.preventDefault();//阻止默认行为
                e.stopPropagation();//阻止冒泡
                if (src.editing)//如果是正在编辑的单元格
                {
                    var targetcell = g.getRowCellWidthKey(g.curEditCell, 'up');
                    if (targetcell) {
                        g.endEdit();
                        targetcell.click();
                    }
        ...
        checkCellLeave: function ()//验证单元格是否离开
        {
            var g = this, p = this.options;

            if (g.editor && g.editor.input) {
                //debugger;
                if (g.editor.input.vo.attr('validate')) {
                    var rowdata = g.getRow(g.editor.editParm.rowindex);
                    var cellObj = g.getCellObj(rowdata, g.editor.editParm.column);

                    //从Das.validator.errorList中删除上次当前单元格对应的编辑器的信息
                    Das.validator.errorList = $.grep(Das.validator.errorList, function (error, i) { return error.element.id != $(cellObj).attr('editor_oldid'); })

                    if (Das.validator.check(g.editor.input.vo[0]) == false)//验证未通过,每验证一次就创建个errorList项，所以要先删除，否则会重复添加
                    {
                        if (!Das.checkout(g.editor.input.vo[0], g)) {
                            g.editor.input.inputText.focus();
                            $(cellObj).attr('editor_oldid', g.editor.input.vo.attr('id'));
                            return false;
                        }
                        else {
                            //给单元格增加验证不通过样式
                            $(cellObj).addClass('l-text-invalid-cell');

                            //增加悬浮提示
                            $(cellObj).removeAttr("title").ligerHideTip();
                            $(cellObj).attr("title", g.editor.input.vo.attr('error')).ligerTip({
                                distanceX: 5,
                                distanceY: -3,
                                auto: true
                            });
						...
						g.removeValidate(cellObj);
                }
            }
        },
		removeValidate: function () {//移除某单元格的验证
			var g = this, p = this.options;
			var cellObj;
			if(arguments.length==1)//传入某单元格
				 cellObj=arguments[0];
			else
			{
				//@arguments:0,itemname;1,rowdata
				cellObj = g.getCellObj(arguments[1], arguments[0]);
				Das.validator.errorList = $.grep(Das.validator.errorList, function (error, i) { return error.element.id != $(cellObj).attr('editor_id'); })
			}
			if ($(cellObj).hasClass("l-text-invalid-cell"))
				$(cellObj).removeClass("l-text-invalid-cell");
        ...
        _onClick: function (e) {
            var obj = (e.target || e.srcElement);
            var g = this, p = this.options;
            var src = g._getSrcElementByEvent(e);//debugger;
            if ((src.out || !src.editing) && p.DasMode)//采集录入模式
            {
                if (g.checkCellLeave() == false) return;
            }
            if (src.out) {
                if (g.editor.editing && !$.liger.win.masking) g.endEdit();
            ...
            {
                g.checkboxallClicked = true;//指定复选框按钮是否点击过
                var row = $(src.hrow);
                ...
                //不包含没有带复选框的行-----周峰
                for (var rowid in g.records) {
                    if (g.WithCheckbox(rowid))//如果此行带有复选框
                    {
                        if (uncheck)
            ...
            else if (src.row && g.enabledCheckbox()) //复选框选择行
            {
                ...
                if (src.checkbox || !selectRowButtonOnly) {
					...
					uncheck&&$(".l-grid-hd-row").removeClass("l-checked");//有1个不选，则去掉全选
                ...
                if (!src.checkbox && src.cell && p.enabledEdit && p.clickToEdit) {
                    if (g._CellEnableEdit(src.cell))//如果允许编辑，周峰修改
                        g._applyEditor(src.cell);
                }
            }
            else if (src.row && !g.enabledCheckbox()) {//debugger;

                //选择行
                if ($(src.row).hasClass("l-selected")) {
                    if (!p.allowUnSelectRow)//不允许反选
                        $(src.row).addClass("l-selected-again");
                    else g.unselect(src.data);
                }
                else {
                    g.select(src.data);
                }
                ...
                if (src.cell && p.enabledEdit && p.clickToEdit) {
                    if (g._CellEnableEdit(src.cell))//如果允许编辑，周峰修改
                    {
                        ...
                        if (p.DasMode) {
                            g.curEditCell = src.cell;//周峰，当前编辑的单元格
                            //$(src.cell).find(':text').focus();
                            /*var column=g._getCellColumn(src.cell.id);
							if(!column.issystem)
							{
								var header = column.display;
								var strLen="";
								...
								var colinfo="名称:["+header+"] 数据类型:("+column.type.toUpperCase()+strLen+ ")";
								$(".l-panel-header-text", g.header).html(colinfo);
							}
							else
                    ...
                    //else
                    //p.DasMode&&$(".l-panel-header-text", g.header).html('');
                }
                //else
                //p.DasMode&&$(".l-panel-header-text", g.header).html('');
            }
                ...
                if (src.first) {
                    if ($("span", src.first).hasClass("l-disabled")) return false;
                    if (g.trigger('toFirst', [g.element]) == false) return false;
                    g.changePage('first');
                }
                else if (src.prev) {
                    if ($("span", src.prev).hasClass("l-disabled")) return false;
                    if (g.trigger('toPrev', [g.element]) == false) return false;
                    g.changePage('prev');
                }
                else if (src.next) {
                    if ($("span", src.next).hasClass("l-disabled")) return false;
                    if (g.trigger('toNext', [g.element]) == false) return false;
                    g.changePage('next');
                }
                else if (src.last) {
                    if ($("span", src.last).hasClass("l-disabled")) return false;
        ...
        _UpDownSelect: function (direction) {
            var g = this, p = this.options;
            if (direction == "up")//上选
            {
                var rows = g.getSelectedRows();
                if (rows.length != 1)//如果不是选择1行
                    return;
                else {
                    if (rows[0]["__previd"] && rows[0]["__previd"] != "-1")
                        g.select(rows[0]["__previd"]);
                    else
                        alert('已到达记录开始！')
                }
            }
            else if (direction == "down")//下选
            {
                var rows = g.getSelectedRows();
                if (rows.length != 1)//如果不是选择1行
                    return;
                    ...
                    if (rows[0]["__nextid"])
                        g.select(rows[0]["__nextid"]);
                    else
                        alert('已到达记录末尾！')
                }
            }
        },
        goTo: function (lx) {//选中某行记录(首行、上一行、下一行、尾行,tips为返回的提示信息)
            var g = this, p = this.options;
            var tips = "";
            switch (lx) {
                case "first":
                ...
                case "end":
                    if (g.rows.length >= 1) {
                        var endrow = g.rows[g.rows.length - 1];
                        if (g.isSelected(endrow))
                            tips = "已到达尾行!";
                        else
                            g.select(endrow);
                    }
        ...
        select: function (rowParm, bSelectTrigger) {//只改变样式，兼容编辑器中的下拉表格
            var g = this, p = this.options;
            var rowdata = g.getRow(rowParm);
            var rowid = rowdata['__id'];
            var rowobj = g.getRowObj(rowid);
            ...
            if (rowobj) {
                ...
                if (p.DasMode && g.columns.length > 0)
                    rowobj.cells[0].focus();
            }
            if (rowobj1) {
                $(rowobj1).addClass("l-selected");
                if (p.DasMode && g.columns.length > 0)
                    rowobj1.cells[0].focus();
            }

            //g.selected[g.selected.length] = rowdata;
            //周峰，应该先判断有没有再添加，比如的p.tree模式下
            if ($.inArray(rowdata, g.selected) == -1)
                g.selected[g.selected.length] = rowdata;
    ...
    $.liger.controls.Grid.prototype.enabledTotal = $.liger.controls.Grid.prototype.isTotalSummary;
    $.liger.controls.Grid.prototype.add = $.liger.controls.Grid.prototype.addRow;
    $.liger.controls.Grid.prototype.update = $.liger.controls.Grid.prototype.updateRow;
    $.liger.controls.Grid.prototype.append = $.liger.controls.Grid.prototype.appendRow;
    $.liger.controls.Grid.prototype.getSelected = $.liger.controls.Grid.prototype.getSelectedRow;
    $.liger.controls.Grid.prototype.getSelecteds = $.liger.controls.Grid.prototype.getSelectedRows;
    $.liger.controls.Grid.prototype.getCheckedRows = $.liger.controls.Grid.prototype.getSelectedRows;
    $.liger.controls.Grid.prototype.getCheckedRowObjs = $.liger.controls.Grid.prototype.getSelectedRowObjs;
    $.liger.controls.Grid.prototype.setOptions = $.liger.controls.Grid.prototype.set;
    $.liger.controls.Grid.prototype.reload = $.liger.controls.Grid.prototype.loadData;
    $.liger.controls.Grid.prototype.refreshSize = $.liger.controls.Grid.prototype._onResize;
