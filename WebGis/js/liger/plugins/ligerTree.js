...
(function ($) {
    $.fn.ligerTree = function (options) {
        ...
        return $.liger.run.call(this, "ligerTree", arguments);
    };

    $.fn.ligerGetTreeManager = function () {

        return $.liger.run.call(this, "ligerGetTreeManager", arguments);
    };
    ...
    $.liger.controls.Tree = function (element, options) {

        $.liger.controls.Tree.base.constructor.call(this, element, options);
    };

    $.liger.controls.Tree.ligerExtend($.liger.core.UIComponent, {
        _init: function () {
            $.liger.controls.Tree.base._init.call(this);
        ...
        expandByLevel: function (level) {
            ...
            $("li", g.tree).each(function () {
                ...
                if ($(this).attr("outlinelevel") == level) {
                    $(">.l-body >.l-expandable-close", $(this)).click();
                } else {
                    $(">.l-body >.l-expandable-open", $(this)).click();
                }
            });
        },
        loadData: function (node, url, parms) {
            var g = this, p = this.options;
            g.loading.show();
            var ajaxtype = parms || p.parms || p.param ? "post" : "get";
            parms = parms || p.parms || p.param || [];
            //请求服务器
            $.ajax({
                type: ajaxtype,
                url: url,
        ...
        //过滤节点（参数string）
        getDataByText: function (selectNodeParm, initTree) {
            var g = this, p = this.options;
            if (!initTree) return;
            if (!selectNodeParm) return g.data;
            var clause = null;
            if (typeof (selectNodeParm) == "string") {
                clause = function (data) {
                    return data.text.indexOf(selectNodeParm) == -1;
                };
            }

            var dataArr = [];
            $("li", initTree).each(function () {

                var treeitem = $(this);
                var treedataindex = parseInt(treeitem.attr("treedataindex"));
                var treenodedata = g._getDataNodeByTreeDataIndex(g.data, treedataindex);
                if (!clause(treenodedata, treedataindex)) {
                    dataArr.push(treenodedata);
                }

            });
            var dataArr = g.grepArr(dataArr);
            dataArr = g.grepArr2(dataArr);
            dataArr = g.grepArr3(dataArr);
            return dataArr;

        },
        grepArr: function (nodes) {
            var g = this, p = this.options;
            for (var i = 0; i < nodes.length; i++) {
                if (!nodes[i].children) break;
                var children = $.unique(nodes[i].children);
                if (children) {
        ...
        grepArr2: function (nodes) {
            var g = this, p = this.options;
            for (var i = 0; i < nodes.length; i++) {
                if (!nodes[i].children) break;
                var child_sub = $.unique(nodes[i].children);
                for (var j = 0; j < child_sub.length; j++) {
                    if (!child_sub[i].children) break;
                    var children = $.unique(child_sub[i].children);
                    if (children) {
                        nodes = $.grep(nodes, function (node, i) {
                            var isIn = $.inArray(node, children) == -1;
                            return isIn;
        ...
        grepArr3: function (nodes) {
            ...
            for (var i = 0; i < nodes.length; i++) {
                ...
                for (var j = 0; j < child_sub.length; j++) {
                    if (!child_sub[i].children) break;
                    child_sub = $.unique(nodes[i].children);
                    for (var k = 0; j < child_sub.length; j++) {
                        var children = $.unique(child_sub[i].children);
                        if (children) {
                            nodes = $.grep(nodes, function (node, i) {
                                var isIn = $.inArray(node, children) == -1;
        ...
        //设置树形节点上移功能
        setNodeMoveUp: function () {
            var g = this, p = this.options;
            g._setNodeMove("up");
        },
        //设置树形节点下移功能
        setNodeMoveDown: function () {
            var g = this, p = this.options;
            g._setNodeMove("down");
        },
        _setNodeMove: function (position) {
            var g = this, p = this.options;
            var nodeArr = g.getChecked();
            var isSilge = true;
            if (!nodeArr) nodeArr = [];
            if (nodeArr.length)
                isSilge = false;
            if (!nodeArr.length)
                nodeArr.push(g.getSelected());
            var nodes = [];
            for (var i = 0; i < nodeArr.length; i++) {
                if (nodeArr[i])
                    nodes.push(nodeArr[i].data);
            }
            for (var i = 0; i < nodes.length; i++) {
                if (!nodes[i].children) break;
                var children = $.unique(nodes[i].children);
                nodes[0].children = $.unique(nodes[0].children);
                if (children) {
                    nodes = $.grep(nodes, function (node, i) {
                        var isIn = $.inArray(node, children) == -1;
                        return isIn;
            ...
            for (var i in nodes) {
                var node = nodes[i];
                var prevNodeindex = null;
                var currNode = $("li[treedataindex=" + node.treedataindex + "]", g.tree);
                if (position == "up") {
                    prevNodeindex = currNode.prev().attr("treedataindex");
                    if (currNode.attr("outlinelevel") == currNode.prev().attr("outlinelevel")) {
                        g.remove(node);
                        g.append(g.getParent(prevNodeindex), [node], prevNodeindex, false)
                    }

                } else {
                    prevNodeindex = currNode.next().attr("treedataindex");
                    if (currNode.attr("outlinelevel") == currNode.next().attr("outlinelevel")) {
                        g.remove(node);
                        g.append(g.getParent(prevNodeindex), [node], prevNodeindex, true)
                    }
                }
                currNode = $("li[treedataindex=" + node.treedataindex + "]", g.tree);
                if (!isSilge)
                    currNode.find(".l-checkbox").removeClass("l-checkbox-unchecked").addClass("l-checkbox-checked");
                else
                    currNode.find(".l-body:first").addClass("l-selected");
            }
