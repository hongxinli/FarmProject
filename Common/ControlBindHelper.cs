...
namespace Common
{
    ...
    public class ControlBindHelper
    {
        ...
        public static void BindHtmlSelectFirstShow(DataTable dt, HtmlSelect select, string _Name, string _ID, string selectValue)
        {
            select.DataSource = dt;
            select.DataTextField = _Name;
            select.DataValueField = _ID;
            ...
            if (!string.IsNullOrEmpty(selectValue))
            {
                ...
                {
                    if (select.Items[i].Value == selectValue)
                    {
        ...
        public static void BindHtmlSelectFirst(DataTable dt, HtmlSelect select, string _Name, string _ID, string selectValue)
        {
            select.DataSource = dt;
            select.DataTextField = _Name;
            select.DataValueField = _ID;
        ...
        public static void BindHtmlSelectt(DataTable dt, HtmlSelect select, string _Name, string _ID, string _Memo)
        {
            select.DataSource = dt;
            select.DataTextField = _Name;
            select.DataValueField = _ID;
        ...
        /// <summary>
        /// 获取aspx页面 服务器控件值，返回model
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="page"></param>
        /// <param name="model"></param>
        /// <returns></returns>
        public static T GetWebControls<T>(Control page, T model)
        {
            int size = HttpContext.Current.Request.Params.Count;
            for (int i = 0; i < size; i++)
            {
                ...
                {
                    HtmlInputText txt = (HtmlInputText)control;
                    if (model.GetType().GetProperty(txt.ID) != null)
                    {
                        if (model.GetType().GetProperty(txt.ID).PropertyType.Name == "String")
                        {
                            model.GetType().GetProperty(txt.ID).SetValue(model, txt.Value.Trim(), null);
                        }
                        else if (model.GetType().GetProperty(txt.ID).PropertyType.Name == "DateTime")//20161230张海娇
                        {
                            model.GetType().GetProperty(txt.ID).SetValue(model, Convert.ToDateTime(txt.Value.Trim()), null);
                        }
                        else
                        {
                            model.GetType().GetProperty(txt.ID).SetValue(model, int.Parse(txt.Value.Trim()), null);
                        }
                    }
                ...
                {
                    TextBox txt = (TextBox)control;
                    model.GetType().GetProperty(txt.ID).SetValue(model, txt.Text.Trim(), null);
                }
                if (control is CheckBoxList)
                {
                    CheckBoxList cbl = (CheckBoxList)control;
                    if (model.GetType().GetProperty(cbl.ID) != null)
                    {
                        model.GetType().GetProperty(cbl.ID).SetValue(model, cbl.SelectedValue, null);
                    }
                }
                if (control is DropDownList)
                {
                    DropDownList ddl = (DropDownList)control;
                    model.GetType().GetProperty(ddl.ID).SetValue(model, ddl.SelectedValue, null);
                }
                if (control is HtmlSelect)
                {
                    HtmlSelect txt = (HtmlSelect)control;
                ...
                if (control is HtmlInputHidden)
                {
                    ...
                    if (model.GetType().GetProperty(txt.ID) != null)
                    {
                        ...
                        else if (model.GetType().GetProperty(txt.ID).PropertyType.Name == "DateTime")
                        {
                            model.GetType().GetProperty(txt.ID).SetValue(model, Convert.ToDateTime(txt.Value.Trim()), null);
                        }
                        else
                ...
                {
                    HtmlInputCheckBox chk = (HtmlInputCheckBox)control;
                    model.GetType().GetProperty(chk.ID).SetValue(model, chk.Checked == true ? 1 : 0, null);
                }
                if (control is HtmlTextArea)
                {
                    HtmlTextArea area = (HtmlTextArea)control;
                    if (model.GetType().GetProperty(area.ID) != null)
                    {
                        model.GetType().GetProperty(area.ID).SetValue(model, area.Value.Trim(), null);
                    }
                }
            }
            return model;
        ...
        public static void SetWebControls<T>(Control page, T model)
        {
            foreach (var Property in model.GetType().GetProperties())
            {
                ...
                string val = Property.GetValue(model, null) != null ? Property.GetValue(model, null).ToString() : "";
                Control control = page.FindControl(key);
                if (control == null) continue;
                if (control is HtmlInputText)
                {
                ...
                {
                    CheckBoxList cbl = (CheckBoxList)control;
                    cbl.SelectedValue = val.ToString();
                }
                if (control is DropDownList)
                {
                    DropDownList ddl = (DropDownList)control;
